...
public class DeliveryController : LegacySellerControllerBase
{
    ...
    public async Task<JsonResult> List(string status = "Processing", int page = 1, int pageSize = 10)
    {
        try
        {
            ...
            {
                return Json(new { success = false, message = await ReadApiErrorAsync(response, "Khong the tai danh sach giao hang.") });
            }

            var payload = await response.Content.ReadFromJsonAsync<ShippingReportApiResponse>(JsonOptions);
            if (payload is null)
            {
                return Json(new { success = false, message = "Khong doc duoc du lieu giao hang tu service." });
            }
    ...
    public async Task<JsonResult> Staffs()
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                return Json(new { success = false, message = await ReadApiErrorAsync(response, "Khong the tai danh sach nhan vien giao hang.") });
            }
    ...
    public async Task<JsonResult> Assign(int orderId, int staffId, string? notes = null, DateTime? expectedPickupAt = null, DateTime? expectedDeliveryAt = null)
    {
        try
        {
            ...
            {
                return Json(new { success = false, message = "Du lieu chi dinh khong hop le." });
            }
            ...
            if (!response.IsSuccessStatusCode)
            {
                return Json(new { success = false, message = await ReadApiErrorAsync(response, "Khong the cap nhat trang thai don hang.") });
            }

            return Json(new
            {
                success = true,
                message = "Da ghi nhan chi dinh tam thoi va chuyen don sang trang thai Dang giao."
            });
        }
        catch (Exception ex)
        {
    ...
    public async Task<JsonResult> UpdateStatus([FromBody] UpdateDeliveryStatusRequest? request)
    {
        try
        {
            ...
            {
                return Json(new { success = false, message = "Du lieu cap nhat trang thai khong hop le." });
            }
